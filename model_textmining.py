...
df = pd.read_csv('D:\\dm\\data_input\\data_input.csv', encoding='latin-1')
#df=df[['v1','v2']]
#df.head(3)
df.columns
...
#print(cp)
cp.to_csv('D:\\dm\\data_output\\data_corpus.csv', index=True, encoding='utf-8')
#insert data result corpus

with open('D:\\dm\\data_output\\data_corpus.csv', 'r') as f:
    reader = csv.reader(f)
...
sk = X,X.shape
sys.stdout = open("D:\dm\data_output\data_vector.txt", "w")
print(sk)
sys.stdout.close()
...
#print(f'print1 {y}')
sys.stdout = open("D:\dm\data_output\data_model.txt", "w")
rpt = print(y)
sys.stdout.close()
#print(y)
df.to_csv('D:\\dm\\data_output\\dataset.csv', index=True, encoding='utf-8')
#y.to_csv('D:\\src\\data_model_clasifikasi.csv', index=True, encoding='utf-8')


## Now we are getting the Spam Column into the model
y=pd.get_dummies(df["label"],drop_first=True)
#print(f'print 2 : {y}')
sys.stdout = open("D:\dm\data_output\data_model_1.txt", "w")
rpt = print(y)
sys.stdout.close()
...
sys.stdout = open("D:\dm\data_output\Gaus_rpt.txt", "w")
...
#akurasi file
sys.stdout = open("D:\dm\data_output\.akurasi_result.csv", "w")
rpt = print(f'Akurasi: {acc}')
sys.stdout.close()

#Generate Report to scv file
sys.stdout = open("D:\dm\data_output\Gaus_rpt.csv", "w")
